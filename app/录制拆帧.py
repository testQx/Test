...
while True:
    ...
    if not device:
        print ('\033[1;33m找不到设备，请检查是否连接USB\033[0m')
        break 
        ...
        if i == 0:
            ...
            os.popen('adb -s ' + device +' shell screencap -p /sdcard/test.png && adb pull /sdcard/test.png ~/Desktop/')
            print("\033[1;32m截图成功，保存至桌面test.png\033[0m")

        elif i == 2:
            print ("录屏开始, 按 control+c 结束录屏")
            try: 
            ...
            print
            print ('\033[1;32m机型:\033[0m', os.popen(cmd_1).read(), '\033[1;32m系统:\033[0m', os.popen(cmd_2).read(), '\033[1;32m分辨率:\033[0m', os.popen(cmd_3).read().split(':')[1])


        elif i == 4:

            apk_name = os.popen('adb -s ' + device +' shell "dumpsys window|grep mCurrentFocus"').readline().split('u0')[1].split('/')[0]
            os.popen('adb -s ' + device +' shell pm clear' + apk_name)
            print ("\033[1;32m缓存清理成功\033[0m")
            print
            ...
            if not device_2:
                ...
                print ('\033[1;32mWiFi连接成功，可拔掉USB，继续使用\033[0m')
                print
            else:
                print ('\033[1;33m存在多个设备号，请检测WiFi是否已连接\033[0m')

        elif i == 6:
            device_2 = os.popen('adb devices').read().split('\n')[2]
            if not device_2:
                ip = os.popen('adb shell ifconfig wlan0').read().split('\n')[1].split(':')[1].split(' ')[0]
                print (os.system('adb disconnect ' + ip + ':6666'))
                print ('\033[1;32mWiFi已断开，请连接USB，继续使用\033[0m')
                print
            else:
                print ('\033[1;33m存在多个设备号，请确认连接WiFi同时，要断开USB\033[0m')
