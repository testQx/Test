    ...
    for i in har_list:
        ...
        if 'chlsj' in i:
            with open(har_path+'/'+str(i), 'r', encoding='utf-8') as f:
                ...
                print(har_ct["request"]["mimeType"])

                parameter_type = 'application/json' if 'urlencoded' in har_ct["request"]["mimeType"] else  har_ct["request"]["mimeType"]
                parameter = dict()
                try:
                    if method in 'POST':
                ...
                if len(parameter) >= 4:
                    ...
                    test_case["address"] = path+'?'+har_ct["query"] if method in 'POST' and har_ct["query"] else path
...
if __name__ == '__main__':
    har = '/Users/bytedance/Documents/lfq/pythontest/data'
